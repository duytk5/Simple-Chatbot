import sys
sys.path.insert(0, '/home/local/DuyTrieu/Work/ChatBotDoc/chatbot-grpc/chatbot')
...
class ChooseSentence(Node):
    ...
    def select_a_choose(self, input):
        for choose in self.list_chooses:
            if utils.check_choose(input, choose[1]):
                if self.attribute != "":
                    ct = choose[0]
...
class MainQuestion(Node):
    ...
    def check_question(self, input):
        if self.question is None:
            return False
        return utils.check_start_question(input, self.question)
